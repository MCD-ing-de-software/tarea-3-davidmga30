...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        # Verificar que en el resultado los valores no tienen espacios
        self.assertEqual(result.loc[0, "name"], "Alice")
        
        # Verificar que las columnas no especificadas no cambiaron
        pdt.assert_series_equal(result["age"], df["age"], check_names=True)
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        # Verificar que el valor extremo (100) fue eliminado
        self.assertNotIn(100, result["age"].values)
        
        # Verificar que al menos uno de los valores no extremos permanece
        self.assertIn(1, result["age"].values)
